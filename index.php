<!-- Acceuil  --> 
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ma Page Web</title>
    <link rel="stylesheet" href="styles.css">
    

    <link href="https://fonts.googleapis.com/css?family=Inter&display=swap" rel="stylesheet" />
    <link href="https://fonts.googleapis.com/css?family=Faster+One&display=swap" rel="stylesheet" />
    <link href="https://fonts.googleapis.com/css?family=Avenir&display=swap" rel="stylesheet" />
    <link href="https://fonts.googleapis.com/css?family=Poppins&display=swap" rel="stylesheet" />
    
</head>
<body>





<!-- Acceuil  --> 

    <!-- menu  -->

    <div class="acceuil" id="acceuilid">

    <header>
   
        <nav>
            <ul>
                <li><a href="#quoid">Quoi</a></li>
                <li><a href="#quiid">Qui</a></li>
                <li><a href="#lieuid">Où</a></li>
                <li><a href="#participerid">Participer</a></li>
            </ul>
        </nav>
     
        <div class="divA">
            <a href="#acceuilid">
                <img alt='Logo' class="logoimg" src='data:image/png;base64,' />
            </a>
        </div>

        
     
    </header>
    
    <!-- FIN menu  -->

   
    <h1>Une conférence autour des métiers </h1>

        <span class="h2">“Wow !”</span>

            <img src="image/eclair.png" alt="eclair" class="eclair1">
            <img src="image/eclair2.png" alt="eclair" class="eclair2">



</div>

    <!-- FIN Acceuil  -->


<!-- Partie QUOI -->

<div class="quoi" id="quoid">

    

        <div class="quoidiv">
            <div class="quoictnr">
                <h2 class="quoititre">Quoi ?</h2>
            </div>
        </div>

    <div class="outer-rectangle">
        <div class="inner-rectangle">
            <div class="inner-title">Evenement Incroyable</div>
            <div class="inner-text">Lorem ipsum dolor sit amet,nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui adi minim veniam, quis nois aute irure dolor in reprehenderit in volup dolore eu</div>
        </div>
        <div class="outer-text">Lorem ipsum dolor sit amet,nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui adi minim veniam, quis nois aute irure dolor in reprehenderit in volup dolore eu</div>
    </div>

</div>

<!-- FIN Partie QUOI -->

<!-- Partie QUI -->

<div class="qui" id="quiid">

    <div class="quidiv">
        <div class="quictnr">
            <h2 class="quititre">Qui ?</h2>
        </div>
    </div>


    <div class="parent2">

        <div class="containercard">
        <div class="card">
            <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
                <image xlink:href="image/img1.jpg" width="100%" height="100%" />
            </svg>
            <div class="card__content">
                <p class="card__title">Jean Balle</p>
                <p class="card__description">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p>
            </div>
        </div>
        <h3 class="Nomprenom">Jean Balle</h3>
    </div>

        <div class="containercard">
        <div class="card">
            <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
                <image xlink:href="image/img2.jpg" width="100%" height="100%" />
            </svg>
            <div class="card__content">
                <p class="card__title">Marc Eur</p>
                <p class="card__description">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p>
            </div>
        </div>
        <h3 class="Nomprenom">Marc Eur</h3>
    </div>

    <div class="containercard">
        <div class="card">
            <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
                <image xlink:href="image/img3.jpg" width="100%" height="100%" />
            </svg>
            <div class="card__content">
                <p class="card__title">claude Patate</p>
                <p class="card__description">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p>
            </div>
        </div>
        <h3 class="Nomprenom">Claude Patate</h3>
    </div>

    <div class="containercard">
        <div class="card">
            <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
                <image xlink:href="image/img4.jpg" width="100%" height="100%" />
            </svg>
            <div class="card__content">
                <p class="card__title">Greg Greg</p>
                <p class="card__description">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p>
            </div>
        </div>
        <h3 class="Nomprenom">Greg Greg</h3>
    </div>

    <div class="containercard">
        <div class="card">
            <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
                <image xlink:href="image/img5.jpg" width="100%" height="100%" />
            </svg>
            <div class="card__content">
                <p class="card__title">Malo Cabon</p>
                <p class="card__description">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p>
            </div>
        </div>
        <h3 class="Nomprenom">Malo Cabon</h3>
    </div>

    <div class="containercard">
        <div class="card">
            <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
                <image xlink:href="image/img6.jpg" width="100%" height="100%" />
            </svg>
            <div class="card__content">
                <p class="card__title">Josef Jojo</p>
                <p class="card__description">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p>
            </div>
        </div>
        <h3 class="Nomprenom">Josef Jojo</h3>
    </div>

    </div> <!-- fin div parent2 -->

    <!-- FIN Partie QUI -->
    


<!-- partie Lieu Date -->

<div class="lieudate">

<div class="lieudate">
    <div class="lieu" id="lieuid">
        <div class="blue-background"> 
            <a href="https://www.google.com/maps/place/Salle+Jean+Renoir/@45.5702516,5.9134502,16.32z/data=!4m6!3m5!1s0x478ba8749a16e813:0x3915daf889e723d3!8m2!3d45.5706528!4d5.9165168!16s%2Fg%2F1tl4by1d?entry=ttu" target="_blank" rel="noopener noreferrer">
                <img class="image-lieu" src="image/lieu.png" alt="Image lieu">
            </a>
            <div class="address">13 Rue Nicolas Parent, Chambéry</div>
            <div class="venue">Salle Jeanrenoir</div>
        </div>
    </div>
</div>


    <div class="date">
    <img src="image/eclair.png" alt="eclair" class="eclairdate">
        <div class="blue-background2">
            <div class="content2">
            <span>Lorem ipsum dolor sit amet,adi minim veniam, quis nois aute irure dolor in reprehenderit in volup dolore eunulla pariatur. Excepteur quis nois aute irure dolor sint occaecat cupidatat quis nois aute irure dolor non proident,quis nois aute irure dolor sunt in culpa qui </span>
            </div>
            <div class="date-titre"> DATES</div>
        </div>
        
    </div>
 
  
</div>



<!-- FIN partie Lieu Date -->

     


<!-- Partie participer -->

<div class="participer" id="participerid">


        <div class="partctnr">
            <h2 class="parttitre">Participer !</h2>
        </div>

       

        <div class="midle">

            <div class="venir">
                <h3 class="titrevenir">Tu veux venir voir ?</h3>
                <div class="venirtxt">
                    <p>Lorem ipsum dolor sit amet, consectetur adi minim veniam, quis nois aute irure dolor in reprehenderit in volup dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa quiLorem ipsum dolor sit amet, consectetur adi minim veniam, quis nois aute irure dolor in reprehenderit in volup dolore eu fugiat nulla pariatur. Excepteur</p>
                </div>
                <button class="reserverBTN"><a href="inscription.php">Réserver</a></button>
            </div>

            <div class="tuas">
                <h3 class="titrevenir">Tu penses avoir un mêtier WOW ?</h3>
                
                <div class="venirtxt">
                    <p>Lorem ipsum dolor sit amet, consectetur adi minim veniam, quis nois aute irure dolor in reprehenderit in volup dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa quiLorem ipsum dolor sit amet, consectetur adi minim veniam, quis nois aute irure dolor in reprehenderit in volup dolore eu fugiat nulla pariatur. Excepteur</p>
                </div>
                <button class="reserverBTN" ><a href="inscription.php">S'inscrire</a></button>
            </div>



        </div><!-- fin div midle -->

</div>

<!-- Fin participer  -->


    <!-- Form -->

<div class="formulaire">
    
    <div class="cont-form">
        <img src="image/eclair.png" alt="eclair" class="eclairform">
        <form action="traitement.php" method="POST">
            <h3 class="contactez">Contactez-nous</h3>
            <p class="aide">Comment pouvons nous vous aider ?</p>
    
            <input type="text" id="nom" name="nom" placeholder="Nom" required>
            <input type="email" id="email" name="email" placeholder="Email" required>
            <input type="text" id="message" name="message" placeholder="Message" required></input>
    
            <input type="submit" value="Envoyer">
        </form>
    </div>
</div>

 <!-- FIN Form -->



<!-- FOOTER -->

   
    

    <footer>
    <div class="svgfooter">   

        <svg  viewBox="0 0 1440 148" magin="opx" xmlns="http://www.w3.org/2000/svg">
        <path d="M325.5 93.5L1439.5 90V0L325.5 93.5Z" fill="#0075FF"/>
        <path d="M1442 147H-1.5L1442 84V147Z" fill="#143498"/>
        <path d="M1311.5 91L-1 147.5V91H1311.5Z" fill="#99B2FF"/>
        </svg>
    </div>  

    <div class="footer">
        <div class="contact">


            <ul class="ulcontact">
                <h3>Contact</h3>
                <li><img class="mail" src="image/mail.svg" alt="logo mail"><a href="#">malo.cabon@gmail.com</a></li>
                <li><img class="tel" src="image/tel.svg" alt="logo tel"><a href="#">07 81 84 50 06</a></li>
                <li><img class="maps" src="image/maps.svg" alt="logo maps"><a href="#">IUT chambéry USMB</a></li>
            </ul>
        </div>

        <div class="Reseaux">

            <div class="links">
               <a href="" > <svg viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg" id="instagram" class="icons-social-media"><path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"></path></svg></a>
               <a href="" > <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" id="twitch" class="icons-social-media"><path d="M391.17,103.47H352.54v109.7h38.63ZM285,103H246.37V212.75H285ZM120.83,0,24.31,91.42V420.58H140.14V512l96.53-91.42h77.25L487.69,256V0ZM449.07,237.75l-77.22,73.12H294.61l-67.6,64v-64H140.14V36.58H449.07Z"></path></svg></a>
               <a href="" > <svg viewBox="0 0 576 512" xmlns="http://www.w3.org/2000/svg" id="youtube" class="icons-social-media"><path d="M549.655 124.083c-6.281-23.65-24.787-42.276-48.284-48.597C458.781 64 288 64 288 64S117.22 64 74.629 75.486c-23.497 6.322-42.003 24.947-48.284 48.597-11.412 42.867-11.412 132.305-11.412 132.305s0 89.438 11.412 132.305c6.281 23.65 24.787 41.5 48.284 47.821C117.22 448 288 448 288 448s170.78 0 213.371-11.486c23.497-6.321 42.003-24.171 48.284-47.821 11.412-42.867 11.412-132.305 11.412-132.305s0-89.438-11.412-132.305zm-317.51 213.508V175.185l142.739 81.205-142.739 81.201z"></path></svg></a></div>
           
             
        </div>

        <div class="imgfoot">
            <img src="image/logoMMI.svg" alt="logommi" class="logommi">

            <img src="image/logoIUT.svg" alt="logoiut" class="logoiut">
        </div>
    </div>
    </footer>

  <!-- FIN FOOTER -->

    
</body>
</html>
