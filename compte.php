<?php
session_start();
if (isset($_SESSION) && isset($_SESSION["logged"])) {
    if($_SESSION['logged'] == true){
        include_once('connexion.php');
        $id = $_SESSION['id'];
        $sql = mysqli_query($connexion, "SELECT * FROM utilisateur LEFT JOIN spectateur ON utilisateur.uid = spectateur.id_utilisateur LEFT JOIN intervenant ON utilisateur.uid = intervenant.id_utilisateur WHERE uid = '$id'");
        if(mysqli_num_rows($sql) == 1) {
            while($row = mysqli_fetch_assoc($sql)){
                $nom_user = $row['nom_utilisateur'];
                $role = $row['type_utilisateur'];
                $mail = $row['uemail'];

               if(isset($row['tribune'])){
                $tribune = $row['tribune'];
               }else if(isset($row['horaire'])){
                $horaire = $row['horaire'];
                }

    }}else {
        header("location: login.php");
    }
}
}else {
    header("location: login.php");
}




?>

<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Mon compte</title>
    <link rel="stylesheet" href="stylerphp.css">
    <link href="https://fonts.googleapis.com/css?family=Faster+One&display=swap" rel="stylesheet" />
</head>
<body>
<header>
   
   <nav>
       <ul>
           <li><a href="#quoid">Quoi</a></li>
           <li><a href="#quiid">Qui</a></li>
           <li><a href="#lieuid">Où</a></li>
           <li><a href="#participerid">Participer</a></li>
       </ul>
   </nav>

   <div class="divA">
 
   <a href="index.php">

    <img alt='Logo' class="logoimg" src='data:image/png;base64,' />
 
   </a>

   </div>
</header>

<div class="main">
    <br><br><br><br><br><br>
    <h1>Bonjour <?=$nom_user?></h1>
    <br>
    <h2>Tu es un <?=$role?></h2>
    <h2>Ton mail est : <?=$mail?></h2>

<?php
    if(isset($tribune)){
        ?>
    <h2>Tu es placé à la tribune du <?=$tribune?></h2>
        <?php
    }else if(isset($horaire)){
        $date = date("d/m/Y", strtotime($horaire));
        $heure = date("h:m", strtotime($horaire));
        ?>

        <h2>Ton passage est prévu pour le <?=$date .' à ' . $heure?></h2>

    <?php
}

?>

    <a href="deconnexion.php">Se déconnecter</a>
</div>

</body>
</html>